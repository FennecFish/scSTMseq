...
I am displaying an example dataset below, and how MANOVA is fitted on thsi dataset.
```{r}
...
ggplot(rep_long, aes(x = pValue)) +
  geom_histogram(bins = 20, color = "black", aes(fill = methods)) +
  facet_wrap(~ methods) +
  theme_minimal() +
...
ggplot(manova.pValue.long, aes(x = pValue)) +
  ...
  facet_wrap(~ methods) +
  theme_minimal() +
  labs(x = "p-value", y = "Frequency", title = "Histogram of p-values under the null using 75th quantile of replicates") +
  scale_fill_manual(values = c("wts" = "skyblue", "mats" = "coral"))
```
...
<!-- ## Simulations Without Sample Variation -->
<!-- ```{r} -->
...
pValue <- readRDS("/proj/milovelab/wu/scLDAseq/res/1MultiSample_SingleResponse_Simulation/simsManual_Manova_pValue_Null_nSample20_nCellType5_noBatch_StromalCell.rds")
pValue <- as.numeric(do.call(rbind, pValue))
hist(pValue, main = "Distribution of MATS pValues under the Null with Simulated Data", breaks = 20)
```
