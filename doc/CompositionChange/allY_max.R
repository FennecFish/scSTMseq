...
for (file_name in files){
  ...
  t1 <- cluster[match(names(time)[time==1], names(cluster))]
  t2 <- cluster[match(names(time)[time==2], names(cluster))]
  ...
  column_names <- paste0("topic_", 1:scSTMobj$settings$dim$K)
  res <- data.frame()
  for (sample in sampleID){
    temp <- data.frame()
    x1 <- t1[names(t1) %in% names(which(scSTMobj$sampleID==sample))]
    x2 <- t2[names(t2) %in% names(which(scSTMobj$sampleID==sample))]
    
    Y <- rbind(table(x1), table(x2))
    
    for (i in 1:length(column_names)) {
      topic <- column_names[i]
      temp[1, topic] <- ifelse(topic %in% colnames(Y), Y[1, topic], NA)
      temp[2, topic] <- ifelse(topic %in% colnames(Y), Y[2, topic], NA)
    }
    cat(file_name, "\n")
    res <- rbind(res, temp)
  }
...
saveRDS(all.Y, file = "res/allY_singleV3_max.rds")
