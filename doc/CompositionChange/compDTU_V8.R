...
files <- list.files(path = "/work/users/e/u/euphyw/scLDAseq/data/simulation/composition_change/V8/scSTM/")

all.Y <- vector(mode = "list")
for (file_name in files){
  scSTMobj <- readRDS(paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/composition_change/V8/scSTM/", file_name))
...
saveRDS(all.Y, file = "allY_V8_scSTM.rds")

all.Y <- readRDS("allY_V7_scSTM.rds")
...
power_error_plot(res, title = "Power plot for CompDTU on scSTMseq Data")

######## directly apply on sims ########
files <- list.files(path = "/work/users/e/u/euphyw/scLDAseq/data/simulation/composition_change/V8/sims/")
all.Y <- vector(mode = "list")
for (file_name in files){
  sims <- readRDS(paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/composition_change/V8/sims/", file_name))
...
saveRDS(all.Y, file = "allY_V8_sims.rds")

all.Y <- readRDS("allY_V8_sims.rds")
compDTU <- function(Y){
  Group <- Y$time
  Group <- factor(Group)
  Y <- Y[,3:ncol(Y)]
