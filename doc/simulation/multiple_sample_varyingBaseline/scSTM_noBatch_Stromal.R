...
sims <- readRDS(paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/MultiSample_VaryingBaseline_noBatch_StromalCell/sims/", file_name))
...
scSTM.mod <- selectModel(sce = sims,sample = "Sample",
                         ...
                         max.em.its = 20, net.max.em.its = 5 , gamma.prior = "L1")
...
scSTM.mod <- selectModel_parallel(sce = sims, sample = "Sample",
                                  K = ngroup, prevalence = ~Time*Response, content = NULL,
                                  N = 5, ts_runs = 30, random_run = 30,
                                  max.em.its = 100, net.max.em.its = 15, gc = gc)

msg <- sprintf("Completed scLDAseq (%d seconds). \n", floor((proc.time()-t1)[3]))
saveRDS(scSTM.mod, file = paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/MultiSample_VaryingBaseline_noBatch_StromalCell/scSTM_noContent_Prevalence_TimeandResponse/", 
