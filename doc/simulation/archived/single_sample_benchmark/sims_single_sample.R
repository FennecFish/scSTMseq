...
sims_scRNA <- function(nsample = 1, nCellType, de.prob, 
  ...
  # 
  #### positive change
  if(nCellType == 5){change.comp <- rep(1/nCellType, nCellType) + c(0.2, -0.15, -0.1, 0.05, 0)}
  if(nCellType == 9){change.comp <- rep(1/nCellType, nCellType) + c(0.1, -0.05, -0.08, 0.03, -0.05, 0.02, 0.02, 0.01, 0)}
...
for(i in 1:dim(nparam)[1]){
  ...
  sims <- sims_scRNA(nsample, nCellType, de.prob, de.facLoc, seed)
  
  saveRDS(sims[[1]], file = paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/single_sample_benchmark/sims/",
                                   "sims_", seed, "_neg_L1_c", nCellType, "_nsample", nsample, ".rds"))
  cat("Generation Completed for L1-neg.\n")
  saveRDS(sims[[2]], file = paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/single_sample_benchmark/sims/",
                                   "sims_", seed, "_pos_L1_c", nCellType, "_nsample", nsample, ".rds"))
  cat("Generation Completed for L1-pos.\n")
  rm(sims)
  
  #### level 2 ####
  de.prob <- runif(nCellType, min = 0.3, max = 0.5)
  ...
  de.facLoc <- runif(nCellType, 0.2, 0.5)
  ...
  de.facLoc <- runif(nCellType, 0.1, 0.2)
