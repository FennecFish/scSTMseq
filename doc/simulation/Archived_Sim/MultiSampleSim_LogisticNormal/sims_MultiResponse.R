...
dir <- "/work/users/e/u/euphyw/scLDAseq/data/simulation/MultiSample_LogisticNormal/NormalGamma_MultiResponse/"
...
sd <- replicate(simplex, diag(c(0,0,1)), simplify = FALSE)
# type = "NullModel"
...
print(list(t1 = true_param$theta$t1, t2= true_param$theta$t2))
rm(sims)

# #########################################
# mean = matrix(rep(0, simplex*A), nrow = A)
# sd <- replicate(4, diag(0.1, A), simplify = FALSE)
# type = "lowVar"
# 
# true_param <- generate_theta(nSample = nSample, nTimepoints = 2, nCellType = nCellType, mean = mean, sd = sd)
# sims <- delta_sim(true_param = true_param, seed = seed, nSample = nSample, 
...
# #########################################
# mean = matrix(rep(0, simplex*A), nrow = A)
# sd <- replicate(simplex, diag(1, A), simplify = FALSE)
# type = "HighVar"
# 
# true_param <- generate_theta(nSample = nSample, nTimepoints = 2, nCellType = nCellType, mean = mean, sd = sd)
# 
...
# #
# # #########################################
# # # ResponseEffect <- rnorm(simplex, mean = 0 , sd = 1)
# # # TimeEffect <- rnorm(simplex, mean = 0 , sd = 1)
# # # 
# # # true_param <- generate_theta(nSample = nSample, nTimepoints = 2, nCellType = nCellType, ResponseEffect = ResponseEffect, TimeEffect = TimeEffect)
# # # sims <- delta_sim(true_param = true_param, seed = seed, nSample = nSample, 
# # #                   nGenes = nGenes, nCellType = nCellType, 
# # #                   de.prob = de.prob, de.facLoc = de.facLoc,
# # #                   batchCells = batchCells, batch.facLoc = batch.facLoc,
...
# # # saveRDS(sims, file = paste0(dir, "nSample", nSample, 
# # #                             "_nCellType", nCellType, "_", save_batch, "_", save_cancer, "/sims/",
# # #                             "sims_", seed, "_ResponseEffect", save_response, "_TimeEffect", save_time, ".rds"))
# # # print(proportion_check(sims))
# # # rm(sims)
# # # 
