...
dat <- read.csv("res/adjRandIndex_MultiSample_VaryingBaseline_noBatch_StromalCell.csv") %>%
...
dat <- read.csv("res/adjRandIndex_MultiSample_VaryingBaseline_noBatch_CancerCell.csv") %>%
  dplyr::select(-matches("cluster$"), matches("cluster$"))
dat$effectSize <- factor(dat$effectSize, levels = paste0("effectSize", c(1, 0.7, 0.4, 0.1)))
dat_long <- dat %>% 
  # rename("scSTMseq" = "scSTM_C_Batch_P_cluster") %>%
...
ggplot(dat_long, aes(x = method, y = adjR, fill = method)) +
  ...
  labs(
    title = "Boxplot of Clustering Accuracy by Methods",
    subtitle = "Batch Effect with One Cancer Cell Group",
    x = "Method",
