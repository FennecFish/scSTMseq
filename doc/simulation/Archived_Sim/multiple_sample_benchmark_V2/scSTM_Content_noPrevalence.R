...
sims <- readRDS(paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/multi_sample_benchmark_V2/sims/", file_name))
# quick qc
sims <- quickPerCellQC(sims, filter=TRUE)

### remove genes with count 0 
sims <- sims[rowSums(counts(sims)) != 0,]
sims <- sims[, colSums(counts(sims)) != 0]
#### feature selection #####
sims <- scuttle::logNormCounts(sims)

dec.p2 <- modelGeneVar(sims)
# feature selection
p2.chosen <- getTopHVGs(dec.p2, n=600)
sims <- sims[p2.chosen,]
...
saveRDS(scSTM.mod, file = paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/multi_sample_benchmark_V2/scSTM_Content_noPrevalance/", 
