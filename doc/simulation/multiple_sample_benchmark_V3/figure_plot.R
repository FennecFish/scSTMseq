...
dat <- read.csv("res/sim_cluster_benchmark/adjRandIndex_multiple_sample_benchmark_V3.csv") %>%
  dplyr::select(-matches("cluster$"), matches("cluster$"))

dat_long <- dat %>% 
  dplyr::select(sim, seed, control, level, nCellType, nSample, scSTM_C_BatchNI_P_cluster, fastTopics_cluster, seurat_cluster, monocle3_cluster) %>%
  rename("scSTMseq" = "scSTM_C_BatchNI_P_cluster") %>%
...
png("res/sim_cluster_benchmark/adjustedRandIndex_lineplot_multiple_sample_benchmark_V3_figure.png", width = 1500, height = 800, res = 150)
ggplot(dat_long, aes(x = level, y = mean_adjR, color = method, group = method)) +
  ...
  labs(
    ...
    subtitle = "Across Different Sample Sizes and Cell Types with SplatPop Based Simulations"
  ) +
...
png("res/sim_cluster_benchmark/adjustedRandIndex_boxplot_multiple_sample_benchmark_V3.png", width = 1200, height = 800, res = 160)
ggplot(dat_long, aes(x = method, y = mean_adjR)) +
  ...
  labs(
    title = "Boxplot of Clustering Accuracy by Methods with SplatPop Based Simulations",
    x = "Method",
    y = "Mean Adjusted Rand Index"
  ) +
  theme_bw() +
