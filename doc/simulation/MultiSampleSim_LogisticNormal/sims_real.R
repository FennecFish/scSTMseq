...
nTimepoints <- 2
nCellType <- 8
A = 3 # number of covariates
batch.rmEffect = TRUE
nGenes = 8000
de.prob <- 0.3
de.facLoc <- 0.5
cancerCellGroup = NULL
batchCells <- rep(c(2000,2000), each = nSample)
...
generate_theta <- function(nSample, nTimepoints = 2, nCellType, gamma){
  Simplex <- nCellType - 1
  A <- 3
  ...
  # for (i in 1:Simplex) {
  #   gamma[[i]] <- diag(mvrnorm(n = A, mu = mean[,i], Sigma = sd[[i]]))
  # }
...
dir <- "/work/users/e/u/euphyw/scLDAseq/data/simulation/MultiSample_LogisticNormal/RealGamma/"
...
## extract gamma from real data
## by fitting a logistic normal on the porportion
## use previously fitted data
realObj <- readRDS("/work/users/e/u/euphyw/scLDAseq/data/PD1_data/PD1_scSTM/scSTM_Content_Batch_Prevalence_TimeResponseInteraction.rds")
realObj <- select_top_scSTM(realObj)
Gamma <- realObj$mu$gamma[-1,]
...
true_param <- generate_theta(nSample = nSample, nTimepoints = 2, nCellType = nCellType, gamma = Gamma)
