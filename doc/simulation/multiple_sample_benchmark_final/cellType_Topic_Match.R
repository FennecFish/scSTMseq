...
for(file_name in files){
  ...
  scSTM_name <- paste0("/work/users/e/u/euphyw/scLDAseq/data/simulation/multi_sample_benchmark_final/scSTM_Content_Batch_Prevalence_Time/scSTM_",set_level,".rds")
...
saveRDS(res, file = "res/de_genes_eval/multi_sample_benchmark_final.rds")

################################################################################
########################### generate plot ######################################
################################################################################

res <- readRDS("res/de_genes_eval/multi_sample_benchmark_final.rds")
...
df_combined$nCellType <- factor(df_combined$nCellType, levels = c("Num_of_CellGroup = 5", "Num_of_CellGroup = 9", 
                                                                  "Num_of_CellGroup = 13" , "Num_of_CellGroup = 17"))
df_combined$DEG_by_Group <- factor(df_combined$DEG_by_Group, levels = paste0("Group", 1:17))
# Create the box plot
png("res/de_genes_eval/multi_sample_benchmark_final_DEGs_proportion_fig.png", width = 1300, height = 600, res =120)
