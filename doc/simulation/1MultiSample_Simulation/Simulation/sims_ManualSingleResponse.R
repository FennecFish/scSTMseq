...
generate_theta <- function(nSample, nTimepoints = 2, nCellType, mean, sd, Response = 1){
  ...
  if(nSample > 1){
    ...
    psi <- mvrnorm(n = 1, mu = rep(0, Simplex),  Sigma= diag(rep(1, Simplex)))
    
    # adding random effect eta = mu + psi
    for (i in 1:nSample) {
      mu[paste0("Sample",i,"_t1"), ] <- mu[paste0("Sample",i,"_t1"), ] + psi
      mu[paste0("Sample",i,"_t2"), ] <- mu[paste0("Sample",i,"_t2"), ] + psi
    }
  }
...
saveRDS(sims, file = paste0(dir, "nSample", nSample,
                            "_nCellType", nCellType, "_", save_batch, "_", save_cancer, "/Manualsims/",
