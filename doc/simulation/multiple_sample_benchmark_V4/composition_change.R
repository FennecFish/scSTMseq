...
# files <- list.files(path = "/work/users/e/u/euphyw/scLDAseq/data/simulation/multi_sample_benchmark_V4/sims/", pattern = "^sims.*c5.*")
...
saveRDS(res_all_file, file = "res/composition_change/estimateEffect_multiple_patient_V4.rds")
...
res_all_file <- readRDS("res/composition_change/estimateEffect_multiple_patient_V4.rds")
...
check_conditions <- function(df) {
  ...
  if (length(significant_indices) == 4 && length(insignificant_indices) == 1) {
    ...
    if (negative_estimates == 2 && positive_estimates == 2) {
      return(TRUE)
    }
  }
  return(FALSE)
