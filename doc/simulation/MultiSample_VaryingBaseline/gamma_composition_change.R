...
for(file_name in files){
  ...
  scSTM_name <- paste0(dir, "scSTM_PooledParam_noContent_Prevalence_TimeandResponse/scSTM_",
                       set_level,".rds")
  ...
  sim_relative_change <- calc_relative_change(model = scSTMobj, gamma_sim = sim_gamma, category = "ResponseR")

  sim_relative_change <- lapply(sim_relative_change, function(mat) {
    R_rc <- (mat["alt_t2",]-mat["alt_t1",])/mat["alt_t1",]
    NR_rc <- (mat["ref_t2",]-mat["ref_t1",])/mat["ref_t1",]
    return(list(Response = R_rc, NonResponse = NR_rc))
  })
  
  sim_relative_change <- list(Response = do.call(rbind, lapply(sim_relative_change, function(x) x$Response)),
                 NonResponse = do.call(rbind, lapply(sim_relative_change, function(x) x$NonResponse)))
  ...
  # Integrate sim and true value
  R_relative_change <- sim_relative_change$Response[,match(R_from_simDat$Topic, colnames(sim_relative_change$Response))]
  NR_relative_change <- sim_relative_change$NonResponse[,match(NR_from_simDat$Topic, colnames(sim_relative_change$NonResponse))]
  ...
  rownames(R_relative_change) <- c("relative_change_simDat",paste0("Sim", 1:nsims))
  rownames(NR_relative_change) <- c("relative_change_simDat",paste0("Sim", 1:nsims))
  res[[set_level]] <- list(Response = R_relative_change, NonResponse = NR_relative_change)
  
  cat(file_name, "\n")
}

saveRDS(res, file = "res/composition_change/MultiSample_VaryingBaseline_PooledParam_noBatch_StromalCell_RelativeChange.rds")

############################# analysis #########################################
res <- readRDS("res/composition_change/MultiSample_VaryingBaseline_PooledParam_noBatch_StromalCell_RelativeChange.rds")
