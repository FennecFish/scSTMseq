...
dir <- "/work/users/e/u/euphyw/scLDAseq/data/simulation/MultiSample_VaryingBaseline/MultiSample_VaryingBaseline_nSample6_nCellType8_noBatch_CancerCell/"
set_level <- sub("^sims_(.*)\\.rds$", "\\1", file_name)


sims <- readRDS(paste0(dir, "sims/", file_name))
seurat.sims <- CreateSeuratObject(counts = counts(sims))
seurat.sims <- SCTransform(seurat.sims, verbose = TRUE, variable.features.n = 1000)
...
saveRDS(seurat.sims, file = paste0(dir, "sctransform/", 
