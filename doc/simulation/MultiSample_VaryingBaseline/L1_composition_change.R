...
for(file_name in files){
  ...
  scSTM_name <- paste0(dir, "scSTM_L1_noContent_Prevalence_TimeandResponse/scSTM_",
                       set_level,".rds")
  ...
  sim_gamma <- scSTMobj$mu$gamma
  rownames(sim_gamma) <- colnames(scSTMobj$settings$covariates$X)
  ...
  t1_ref <- sim_gamma["(Intercept)",]
  t2_ref <- sim_gamma["(Intercept)",] + sim_gamma["TimeTime2",]
  t1_alt <- sim_gamma["(Intercept)",] + sim_gamma["ResponseR",]
  t2_alt <- sim_gamma["(Intercept)",] + sim_gamma["ResponseR",] + sim_gamma["TimeTime2",] + sim_gamma["TimeTime2:ResponseR",]
  
  combined_results <- list(t1_ref = t1_ref, t2_ref = t2_ref, t1_alt = t1_alt, t2_alt = t2_alt)
  combined_results <- do.call(rbind, combined_results)
  combined_results <- cbind(0, combined_results)
  map_to_simplx <- function(x) {
    exp(x - log(sum(exp(x))))
  }
  
  simplx_results <- apply(combined_results, 1, function(row) {
    t(as.matrix(map_to_simplx(row)))
  })
  rownames(simplx_results) <- paste0("topic_", 1:K)
...
saveRDS(res, file = "res/composition_change/MultiSample_VaryingBaseline_noBatch_StromalCell_ProportionChange.rds")

############################# analysis #########################################
res <- readRDS("res/composition_change/MultiSample_VaryingBaseline_noBatch_StromalCell.rds")
...
mean_sd$within_range <- with(mean_sd, 
                             relative_change_simDat >= (mean - sd) & 
                               relative_change_simDat <= (mean + sd))
