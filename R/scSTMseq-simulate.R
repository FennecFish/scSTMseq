#' Simulate scSTMseq-style single-cell data (Extended from Splatter)
#'
#' This function wraps and modifies `splatSimulate()` from Splatter to support
#' paired/time-series simulations with cell-level covariates and sample-level variation.
#' It uses Splatter's original helper functions and replaces only the relevant internal steps.
#'
#' @param params A SplatParams object.
#' @param method Simulation method (default = "groups").
#' @param sparsify Logical; whether to sparsify assays.
#' @param verbose Logical; print progress messages.
#' @param cancerCellGroup Integer; group index for heterogeneous DE.
#' @param true_param List; expected values of theta and optional gamma/psi.
#' @param cancerCell.facLoc Mean of sample-level cancer group shift.
#' @param cancerCell.facScale SD of sample-level cancer group shift.
#' @param ... Passed to `setParams()`.
#'
#' @return A SingleCellExperiment with simulated data.
#' @export
scSTMseqSimulate <- function(params = newSTMParams(),
                             method = "groups",
                             sparsify = TRUE,
                             verbose = TRUE,
  ...
  nSample <- nBatches / 2
  nTime <- if (is.list(group.prob)) length(group.prob) else 1
  ...
  batches <- unlist(lapply(seq_len(nBatches), function(i) rep(i, batch.cells[i])))
  ...
  if (!is.null(true_param)) {
    if (nSample > 1) {
      meta_ids <- rownames(true_param$theta[[1]])
      response_info <- data.frame(
        Sample = sapply(strsplit(meta_ids, "_"), `[`, 1),
        Response = sapply(strsplit(meta_ids, "_"), `[`, 2)
      )
      colData(sim)$Response <- response_info$Response[match(colData(sim)$Sample, response_info$Sample)]
    } else {
      colData(sim)$Response <- if (all(true_param$gamma == 0)) "Response" else "nonResponse"
    }
  }

  withr::with_seed(seed, {
    if (method != "single") {
      groups <- unlist(lapply(seq_len(nSample), function(i) {
        g1 <- sample(seq_len(nGroups), batch.cells[i], prob = group.prob[[1]][i,], replace = TRUE)
        g2 <- sample(seq_len(nGroups), batch.cells[i + nSample], prob = group.prob[[2]][i,], replace = TRUE)
        c(g1, g2)
      }))
      colData(sim)$Group <- factor(group.names[groups], levels = group.names)
    }

    if (verbose) message("Simulating core components...")
    sim <- splatSimLibSizes(sim, params)
    sim <- splatSimGeneMeans(sim, params)

    if (nBatches > 1) {
      sim <- scSTMseqSimBatchEffects(sim, params, nSample)
    }

    sim <- splatSimBatchCellMeans(sim, params)
    sim <- scSTMseqSimGroupDE(sim, params, cancerCellGroup, cancerCell.facLoc, cancerCell.facScale)
    sim <- scSTMseqSimGroupCellMeans(sim, params, cancerCellGroup)
  ...
  if (sparsify) {
    ...
    assays(sim) <- sparsifyMatrices(assays(sim), auto = TRUE, verbose = verbose)
  }

  if (verbose) message("scSTMseqSimulate: Done.")
  return(sim)
}
...
#'
#' @keywords internal
scSTMseqSimBatchEffects <- function(sim, params, nSample) {
    nGenes <- getParam(params, "nGenes")
...
scSTMseqSimGroupDE <- function(sim, params, cancerCellGroup, cancerCell.facLoc, cancerCell.facScale) {
    nGenes <- getParam(params, "nGenes")
...
#' @rdname splatSimCellMeans
#' @importFrom SummarizedExperiment rowData colData assays assays<-
scSTMseqSimGroupCellMeans <- function(sim, params, cancerCellGroup) {
    nGroups <- getParam(params, "nGroups")
    cell.names <- colData(sim)$Cell
    gene.names <- rowData(sim)$Gene
